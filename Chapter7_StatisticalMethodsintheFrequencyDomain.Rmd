---
title: "Chapter7_StatisticalMethodsintheFrequencyDomain"
...
```
### Example7.1 Predicting Lake Shasta In???ow 
```{r}
...
abline(h=0)
```
### Example7.2 An Infrasonic Signal from a Nuclear Explosion
```{r}
attach(beamd)
tau    = rep(0,3) 
...
Time  = time(Y)
colnames(Y) = c('sensor1', 'sensor2', 'sensor3', 'beam') # added
...
grid(); lines(Y[,3])
plot(Time, Y[,4], type='n')  
grid(); lines(Y[,4]) 
title(xlab="Time", outer=TRUE)
```
### Example7.4 Detecting the Infrasonic Signal Using ANOPOW
```{r}
attach(beamd)
L = 9
fdr = .001 
...
### Example7.5 Estimating the Random Infrasonic Signal
```{r}
attach(beamd)
L  = 9
M  = 100 
...
### Example7.6 MeansTestforthefMRIData 
```{r}
...
```
### Example7.7 Analysis of Power Tests for the fMRI Series
```{r}
n      = 128
n.freq = 1 + n/2  
Fr     = (0:(n.freq-1))/n  
...
```
### Example7.8 SimultaneousInferenceforthefMRISeries
```{r}
n      = 128
n.freq = 1 + n/2  
Fr     = (0:(n.freq-1))/n
...
```
### Example7.9 EqualityofMeansandSpectralMatrices 
```{r}
P = 1:1024 
S = P+1024
N = 8
...
```
## 7.7 DiscriminantandClusterAnalysis 
### Example7.10 DiscriminantAnalysisUsingAmplitudes 
```{r}
P     = 1:1024
S     = P+1024
...
round(Posterior, 3)   
```
### Example7.11 DiscriminantAnalysisonSeismicData 
```{r}
P = 1:1024
S = P+1024
p.dim = 2
...
text(KLDiff[c(1,2,3,7,14)]+.075, BDiff[c(1,2,3,7,14)], label=names(eqexp[c(1,2,3,7,14)]), cex=.7)
```
### Example7.12 ClusterAnalysisforEarthquakesandExplosions 
```{r}
library(cluster)
P = 1:1024
S = P+1024
...
```
## 7.8 PrincipalComponentsandFactorAnalysis
### Example7.13 PrincipalComponentAnalysisofthefMRIData
```{r}
n = 128
...
```
## FactorAnalysis
### Example7.14 SingleFactorAnalysisofthefMRIData 
```{r}
bhat = sqrt(lam[1])*evec[,1]
Dhat = Re(diag(fxx[,,4] - bhat%*%Conj(t(bhat))))
res = Mod(fxx[,,4] - Dhat - bhat%*%Conj(t(bhat)))
```
### Example7.15 GovernmentSpending,PrivateInvestment,andUnemployment 
```{r}
...
```
## 7.9 TheSpectralEnvelope 
### Example7.18 OptimalTransformationsforFinancialData:NYSEReturns
```{r}
u   = astsa::nyse             
