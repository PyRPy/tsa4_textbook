# Chapter 11 TIME SERIES REGRESSION MODELS.R ------------------------------
...
# Exhibit 11.5
acf(as.vector(diff(diff(window(log(airmiles),end=c(2001,8)),12))),
    lag.max=48)
...
# those in the book, because R uses the plus convention in the MA 
# parameterization.
...
n=500
for (i in 1:B) {
  x=cumsum(arima.sim(model=list(ma=.8),n=n))
...
# The as.numeric function strips the time series attribute from the time 
# series.
# This is done to nullify the default way the ccf function plots the 
# cross-correlations.
