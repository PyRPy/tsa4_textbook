...
```
### Example6.17 Normal HMM-S&P 500 Weekly Returns
```{r}
library(depmixS4)
...
```
### Example6.18 Switching AR-In???uenza Mortality 
```{r}
library(MSwM)
set.seed(90210)
dflu  = diff(flu)
...
```
### Example6.22 Analysisof the In???uenza Data
```{r}
...
### Example6.23 Analysis of the New York Stock Exchange Returns
```{r}
y   = log(nyse^2) 
num =length(y)
...
```
### Example6.24 Analysis of the U.S.GNP Growth Rate
```{r}
...
legend('topleft', c('true smoother', 'data', 'posterior mean', '95% of draws'), lty=1, 
         ...
         bg='white' )  
```
### Example6.27 Structural Model
* Consider the Johnson & Johnson quarterly earnings per share series that was discussedinExample6.10
```{r}
